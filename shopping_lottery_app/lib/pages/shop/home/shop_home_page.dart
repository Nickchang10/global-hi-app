...
/// ✅ ShopHomePage（商城首頁｜完整版｜可編譯）
/// ------------------------------------------------------------
/// 修正重點：
/// - ✅ AppStartupGate 直接內嵌在本檔案
/// - ✅ Firestore 有資料就顯示 banners/categories/products，沒有就顯示 fallback
/// - ✅ 全部 withOpacity -> withValues(alpha: ...)（避免 deprecated_member_use）
/// - ✅ _pushNamedSafe 改同步（避免 use_build_context_synchronously）
/// - ✅ const items（避免 prefer_const_declarations）
/// ------------------------------------------------------------
...
class _ShopHomePageState extends State<ShopHomePage> {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                '尚未設定 Banner（建立 banners collection：title/imageUrl/isActive/sort）',
                style: TextStyle(
  ...
  Widget _categorySection() {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: q.snapshots(),
      builder: (context, snap) {
        if (snap.hasError) return _cardError('分類讀取失敗：${snap.error}');
        if (!snap.hasData) return _cardLoading(height: 120);

        final docs = snap.data!.docs;
        if (docs.isEmpty) return _categoriesFallback();
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      final d = docs[i].data();
                      final name = (d['name'] ?? '分類').toString();
                      final id = docs[i].id;

                      return InkWell(
                        borderRadius: BorderRadius.circular(14),
                        onTap: () => _pushNamedSafe(context, '/category/$id'),
                        child: Container(
                          width: 120,
  ...
  Widget _categoriesFallback() {
    const items = [
      {'name': '穿戴裝置', 'route': '/products'},
      {'name': '配件周邊', 'route': '/products'},
      {'name': '健康服務', 'route': '/health'},
      {'name': '優惠活動', 'route': '/promotion'},
    ];
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                for (final it in items)
                  InkWell(
                    borderRadius: BorderRadius.circular(14),
                    onTap: () =>
                        _pushNamedSafe(context, it['route']!.toString()),
                    child: Container(
                      width: 160,
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
            ...
            Text(
              '（尚未設定 categories collection，先用 fallback）',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            Text(
              '尚未設定 products collection（name/price/imageUrl/isActive/homePinned）',
