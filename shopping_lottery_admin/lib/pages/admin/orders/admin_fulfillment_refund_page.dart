...
// ------------------------------------------------------------
// - 批次選取訂單（依查詢結果）
// - 批次更新：status（例如：pending/paid/shipped/done/cancelled）
// - 批次補寫：note（adminNote）
// - 批次補寫：trackingNo（物流單號）
// - 內建防呆：沒有選到訂單不允許送出
// - ✅ FIX: control_flow_in_finally（finally 裡不使用 return）
//
// ✅ Firestore orders 建議欄位：
// orders/{orderId} {
//   status: string,
//   adminNote: string,
//   trackingNo: string,
...
//
// ✅ 呼叫：
// Navigator.pushNamed(context, '/admin_orders_batch_ops');
//
...
class _AdminOrdersBatchOpsPageState extends State<AdminOrdersBatchOpsPage> {
  ...
  Query<Map<String, dynamic>> _query() {
    // createdAt 若你有索引可以 orderBy('createdAt')
  ...
  // ===========================================================
  // Batch operations (✅ FIX: no return inside finally)
  // ===========================================================
  Future<void> _applyBatchUpdate() async {
    ...
    Object? caughtError;
    try {
      // Firestore batch 500 限制 → 分段提交
      final ids = _selected.toList(growable: false);

      const chunkSize = 450; // 留 buffer
    ...
    // ✅ finally 後再處理結果（避免 finally 內 return）
    if (caughtError != null) {
      _snack('批次更新失敗：$caughtError');
      return;
    }
