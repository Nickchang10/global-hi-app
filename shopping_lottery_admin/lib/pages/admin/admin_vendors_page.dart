...
//
// ✅ AdminCustomHomePage v6.6 Final（首頁自訂區塊管理｜最終完整版｜可編譯可用）
// ------------------------------------------------------------
// ✅ 修正：curly_braces_in_flow_control_structures
// - 把 StreamBuilder 裡的「單行 if return」改成「有大括號」的 block
//
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          // ✅ 修正：加上大括號
          if (docs.isEmpty) {
            return const Center(child: Text('尚無區塊'));
          }
            ...
            children: [
              ReorderableListView.builder(
                ...
                itemBuilder: (context, i) {
                  ...
                  final updated = _fmt(data['updatedAt']);
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: PopupMenuButton<String>(
                        onSelected: (v) async {
                          if (v == 'edit') {
                            await _edit(docs[i].id);
                          }
                          if (v == 'toggle') {
                            await _toggleActive(docs[i]);
                          }
                          if (v == 'delete') {
                            await _delete(docs[i]);
                          }
                        },
                        itemBuilder: (_) => [
...
class _CustomHomeEditSheetState extends State<_CustomHomeEditSheet> {
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    // ✅ 關鍵：putData 需要 Uint8List
    final Uint8List? bytes = f.bytes ?? await _readAllBytes(f.readStream);
    if (bytes == null) {
    ...
    try {
      ...
      final group = _groupCtrl.text.trim().isEmpty
          ? 'main'
          : _groupCtrl.text.trim();
      final safeName = (f.name.isEmpty ? 'image' : f.name).replaceAll(' ', '_');
      final path =
          'custom_home/$group/${DateTime.now().millisecondsSinceEpoch}_$safeName';
