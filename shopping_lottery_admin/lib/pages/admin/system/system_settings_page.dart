...
//
// ✅ SystemSettingsPage（系統設定｜可編譯完整版）
// ------------------------------------------------------------
...
class _SystemSettingsPageState extends State<SystemSettingsPage> {
  ...
  Future<void> _save() async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('系統設定已儲存')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
  ...
  Future<void> _editRawJson() async {
    ...
    try {
      final parsed = json.decode(ctrl.text);
      if (parsed is! Map) {
        throw Exception('必須是 JSON object（{...}）');
      }
      final next = Map<String, dynamic>.from(parsed);
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('已套用到本地（記得按儲存）')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('JSON 格式錯誤：$e')));
    }
  }
