...
//
// ✅ VendorManagementPage（最終完整版｜已修正 use_build_context_synchronously / control_flow_in_finally / deprecated）
// ------------------------------------------------------------
...
class _VendorManagementPageState extends State<VendorManagementPage> {
  ...
  Future<void> _load({bool refresh = false}) async {
    ...
    try {
      ...
      // ✅ finally 不要 return（避免 control_flow_in_finally）
  ...
  Future<void> _openCreate() async {
    final ok = await Navigator.of(
      context,
    ).push<bool>(MaterialPageRoute(builder: (_) => const _VendorEditPage()));
    if (!mounted) return;
