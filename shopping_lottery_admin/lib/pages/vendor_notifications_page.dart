// lib/pages/vendor_notifications_page.dart
//
// ✅ VendorNotificationsPage（最終穩定完整版｜可編譯｜Vendor Only｜通知中心｜即時監聽｜已讀/未讀｜搜尋｜批次已讀｜匯出CSV(複製)｜Web+App）
//
// 核心原則：
// - 只顯示 vendorId == 自己 vendorId 的通知
// - 與主後台 / 系統通知（同集合）即時連動
//
// Firestore 建議：vendor_notifications/{notificationId}
//   - vendorId: String
//   - title: String
//   - body: String
...
//
// 注意：
// - where(vendorId)+where(isRead)+orderBy(createdAt) 可能需要複合索引；若遇錯誤 console 會提供索引建立連結。
...
class VendorNotificationsPage extends StatefulWidget {
  const VendorNotificationsPage({
    ...
    this.collection = 'vendor_notifications',
  });
...
class _VendorNotificationsPageState extends State<VendorNotificationsPage> {
  ...
  Future<void> _batchMarkRead(List<_NotiRow> rows, bool read) async {
    ...
    await _setBusy(true, label: read ? '批次已讀...' : '批次未讀...');
    try {
