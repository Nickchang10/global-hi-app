...
//
// ✅ OrderDetailPanel（完整版・可編譯強化版｜Lint Clean）
// ------------------------------------------------------------
// - 修正：curly_braces_in_flow_control_structures（所有 if 單行改成 {}）
// - 修正：control_flow_in_finally（finally 不再 return）
// - 修正：withOpacity deprecated（改用 withAlpha）
// - 修正：DropdownButtonFormField.value deprecated（改用 initialValue + Key）
// ------------------------------------------------------------
...
class _OrderDetailPanelState extends State<OrderDetailPanel> {
  ...
  String _norm(dynamic v) {
    return _s(
      v,
    ).toLowerCase().replaceAll(' ', '').replaceAll('_', '').replaceAll('-', '');
  }
  ...
  String get _orderId {
    ...
    return _s(widget.order['orderId']);
  }

  List<String> get _vendorIds {
    return _asList(
      widget.order['vendorIds'],
  ...
  void didUpdateWidget(covariant OrderDetailPanel oldWidget) {
    ...
    final oldId = _s(oldWidget.order['id']);
    final newId = _s(widget.order['id']);
    if (oldId != newId ||
  ...
  Color _toneForStatus(BuildContext context, String s) {
    ...
    if (key == 'pendingpayment' || key == 'codpending') {
  ...
  double _pickAmount(Map<String, dynamic> o) {
    ...
    if (direct != null) {
      return (num.tryParse('$direct') ?? 0).toDouble();
    }
  ...
  Future<void> _appendUpdate({
    ...
    final oid = _orderId;
    if (oid.isEmpty) {
      throw StateError('orderId missing');
    }
  ...
  Future<void> _saveStatus() async {
    ...
    try {
      ...
      if (mounted) {
        _showSnack(messenger, '已更新狀態：$target');
      }
    } catch (e) {
      if (mounted) {
        setState(() {
  ...
  Future<void> _saveShipping() async {
    ...
    if (!(_vendorScopedAllowed || widget.isAdmin)) {
      setState(() {
        _error = '權限不足：此訂單不屬於你的 vendor scope，無法更新物流資訊';
      });
      return;
    }
    ...
    try {
      ...
      await _appendUpdate(
        ...
        extra: {
          'by': widget.isAdmin ? 'admin' : 'vendor',
          if (!widget.isAdmin) 'vendorId': _s(widget.vendorId),
          if (_s(shipping['carrier']).isNotEmpty)
            'carrier': shipping['carrier'],
          if (_s(shipping['trackingNo']).isNotEmpty)
  ...
  Widget build(BuildContext context) {
    ...
    final currency = _s(widget.order['currency']).isEmpty
        ? 'TWD'
        : _s(widget.order['currency']);
                      ...
                      children: [
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: statusTone.withAlpha(_a(0.12)),
                        ...
                        Wrap(
                          ...
                          children: [
                            ...
                            _kvChip('付款', payStatus.isEmpty ? '—' : payStatus),
                            _kvChip(
                              'Provider',
                              payProvider.isEmpty ? '—' : payProvider,
                            ),
                            _kvChip(
                              'Method',
                              payMethod.isEmpty ? '—' : payMethod,
                            ),
                          ],
                        ),
                          ...
                          children: [
                            Expanded(
                              child: DropdownButtonFormField<String>(
                                key: ValueKey('statusField_$oid'),
                                initialValue: _status.trim().isEmpty
                                    ? 'unknown'
                                    : _status,
                            ...
                            FilledButton.icon(
                              onPressed: (!canEdit || _savingStatus)
                                  ? null
                                  : _saveStatus,
                              icon: _savingStatus
                                  ? const SizedBox(
                        ...
                        if (!widget.isAdmin)
                          Padding(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(
                                color: Colors.black.withAlpha(_a(0.55)),
                              ),
                            ),
                          ),
                      ],
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _kvRow(
                          '電話',
                          buyerPhone.isEmpty ? '—' : buyerPhone,
                          onCopy: buyerPhone.isEmpty
                              ? null
                              : () => _copy(buyerPhone),
                        ),
                          ...
                          Column(
                            children: items.map((it) {
                              ...
                              final subtotal = _toDouble(
                                m['subtotal'] ?? (price * qty),
                              );
                              final pid = _s(m['productId']).isNotEmpty
                                  ? _s(m['productId'])
                                  : _s(m['id']);
                              final sku = _s(m['sku']);
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            const SizedBox(width: 10),
                            OutlinedButton.icon(
                              onPressed: _s(shipping['trackingNo']).isEmpty
                                  ? null
                                  : () => _copy(
                                      _s(shipping['trackingNo']),
                                      done: '已複製追蹤碼',
                                    ),
                          ...
                          children: [
                            Expanded(
                              child: FilledButton.icon(
                                onPressed: (!canEdit || _savingShipping)
                                    ? null
                                    : _saveShipping,
                        ...
                        Text(
                          widget.isAdmin
                              ...
                              : (canEdit
                                    ? 'Vendor 可更新此訂單的物流資訊。'
                                    : 'Vendor 無法更新：此訂單不在你的 vendorIds 範圍內。'),
                          style: const TextStyle(
                            color: Colors.black54,
                            fontSize: 12,
                          ),
  ...
  Widget _kvChip(String k, String v) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.black.withAlpha(_a(0.04)),
        borderRadius: BorderRadius.circular(999),
        border: Border.all(color: Colors.black.withAlpha(_a(0.06))),
      ),
      child: Text(
        '$k：$v',
